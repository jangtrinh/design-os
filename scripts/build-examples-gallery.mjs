...
/** GitHub honours <picture> media queries, so each artifact shows in the reader's own theme. */
function picture(kind, name, title, width) {
  const base = `docs/images/examples/${kind}s/${name}`;
  return `<picture><source media="(prefers-color-scheme: dark)" srcset="${base}-dark.png"><img src="${base}-light.png" width="${width}" alt="${escapeHtml(title)}"></picture>`;
}
