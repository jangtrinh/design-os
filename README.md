...
| Date | Change | Commit |
|---|---|---|
| 2026-08-19 | **FloorFinding schema v1 + `ui gate coverage`** — findings gain declared repair fields (`expected`/`actual`/`fixHint`/`repairScope`) and the gate grows a coverage registry (82 checks, per-project activity, test-paired against the family sources) — the evidence base for tractability routing and strict patch validation | `#198` |
