...
describe(PRODUCT_CONTEXT_SUITE, () => {
  ...
  it("replay errors block before Flow while replay and direct warnings form the raw union", () => {
    ...
    expect(json(mismatch, "mismatched project-flow")).toEqual({ ok: false, command: "product-context project-flow", error: { code: "BAD_PRODUCT_ATLAS", message: expect.not.stringMatching(/^[A-Z][A-Z_]+$/) } });
