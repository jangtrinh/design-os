...
export const COMMAND_SIGNATURES: Readonly<Record<string, CommandSchema>> = {
  ...
  chart: {
    ...
    subcommands: {
      lint: {
        summary: "Lint self-contained chart HTML deterministically",
        positionals: [htmlFile("Self-contained chart HTML file to lint")],
        flags: [],
    ...
    subcommands: {
      reconcile: {
        ...
        flags: [
          ...
          { name: "skip", type: "string", summary: "Acknowledge a pending target (nodeId) so the cursor may pass it — CLI-only, audited, standalone (no --apply/--dry-run)" },
