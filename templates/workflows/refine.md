...
4. **Per-pass loop** — repeat for each pass until the budget is exhausted or an early
   ...
   If the failing evidence belongs to an existing T6 Canvas UI effect, invoke
   `design-os-canvas-effect` before editing. Repair the fallback, teardown, or tenant
   contract without inventing a new effect.
