...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    const dive = { kind: 'dive', si: i, clip: s.clip, clipM: s.clipMobile, still: s.still, stillM: s.stillMobile,
                   ...
                   rendererKind: s.renderer || 'video', framesDir: s.frames, frameCount: s.frameCount, reverse: !!s.reverse, range: s.range };
    SEGMENTS.push(dive);
    sectionSeg[i] = dive;
  ...
  function imageSequenceRenderer(s) {
    ...
    function requestFrame(i) {
      ...
      img.onload = () => {
        ...
        if (canvas && !firstPainted && frames[lastIdx]) {
          drawFrame(lastIdx); firstPainted = true; s.el.classList.add('has-clip');
        }
      };
    ...
    function drawFrame(i) {
      const img = frames[i]; if (!img || !ctx2d) return;
    ...
    return {
      ...
      draw(progress) {
        ...
        releaseFar(idx);
        if (frames[idx]) { drawFrame(idx); return; }
        // Fallback to the nearest already-loaded frame so a fast flick never
        // flashes blank while its window is still in flight.
        for (let d = 1; d <= 5; d++) {
          if (frames[idx - d]) { drawFrame(idx - d); return; }
          if (frames[idx + d]) { drawFrame(idx + d); return; }
        }
      },
      poster() { const lo = s.range ? s.range[0] : 0, hi = s.range ? s.range[1] : N - 1;
                 return s.still || (N ? frameUrl(s.reverse ? hi : lo) : null); },
