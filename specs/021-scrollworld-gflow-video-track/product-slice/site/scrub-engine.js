/* ============================================================================
   ...
   RENDERER (per-DIVE-section opt-in; default 'video' — every existing config is
   byte-for-byte unaffected. Connectors always use the video renderer.)
     sections[i].renderer     'video' (default) | 'imageseq'
     sections[i].frames       'imageseq' only — dir (trailing slash) of frame files
                               named 0001.webp … NNNN.webp
     sections[i].frameCount   'imageseq' only — N, the frame count in that dir
     'video'    → video-scrub-renderer: the blob-seek <video> path this file always
                  had (verbatim — extracted into an object, behavior unchanged).
     'imageseq' → image-sequence-renderer: preloads WebP frames onto a <canvas>,
                  staged (first ~10 eager on load(), then a ±5 window around the
                  scrubbed frame index as progress moves), drawing
                  frames[round(progress*(N-1))] with manual cover-fit crop math
                  (<canvas> has no CSS object-fit).
   Internal per-segment renderer object: { load(), draw(progress), poster(), busy(),
     prime() }. draw() receives the engine's already-smoothed 0..1 progress (the
   scroll→time lerp/easing stays shell-side, shared across renderers). busy()/prime()
   are scheduling hooks the video path needs (seek-coalescing, iOS gesture-priming);
   imageseq no-ops both since canvas draws are synchronous.

   es-debt: this file crosses the project's 200-line/file guidance (pre-refactor size
   preserved, two renderers added in place). Splitting the renderers into their own
   modules wasn't requested for this slice and adds risk against the "keep the video
   path verbatim" constraint. Ceiling: split when a 3rd renderer lands or a review
   asks for it.
   ========================================================================== */
...
function mountScrollWorld(container, config) {
  ...
  // sectionSeg is a mount-local section-index -> dive-segment map. Previously this
  // wrote `s._seg = dive` directly onto the caller's SECTIONS entries — a shared
  // config object passed into mountScrollWorld — so two mounts sharing the same
  // config (e.g. re-mounting on route change, or two instances in one page) would
  // clobber each other's `_seg` pointer. Keeping the map mount-local fixes that.
  const SEGMENTS = [];
  const sectionSeg = new Array(N);
  SECTIONS.forEach((s, i) => {
    const dive = { kind: 'dive', si: i, clip: s.clip, clipM: s.clipMobile, still: s.still, stillM: s.stillMobile,
                   accent: s.accent, w: s.scroll || DIVE_W, linger: s.linger || 0,
                   rendererKind: s.renderer || 'video', framesDir: s.frames, framesDirLQ: s.framesLQ, frameCount: s.frameCount, reverse: !!s.reverse, range: s.range };
    SEGMENTS.push(dive);
    sectionSeg[i] = dive;
  ...
  SEGMENTS.forEach(s => {
    ...
    s.renderer = createRenderer(s.rendererKind, s);
    // still/stillM win when given (unchanged for the video path); imageseq sections
    // may omit `still` entirely and fall back to the renderer's own poster (frame 0001).
    const poster = (isMobile() && s.stillM) ? s.stillM : (s.still || (s.renderer.poster && s.renderer.poster()));
  ...
  function layout() {
    ...
    totalW = off;
    // Renderer-owned canvases need their backing store resized to the new scene
    // box; a no-op before load() (canvas doesn't exist yet) or for the video path
    // (object-fit:cover on the <video> element needs no JS resize).
    SEGMENTS.forEach(s => { if (s.renderer && s.renderer.resize) s.renderer.resize(); });
    track.style.height = (totalW * vh + vh) + 'px';   // +1vh so the last flight completes
    read();
  }

  function jumpTo(i) {
    const seg = sectionSeg[i];
    window.scrollTo({ top: seg.start + (seg.end - seg.start) * 0.5, behavior: reduce ? 'auto' : 'smooth' });
  }

  // ---- renderers ----
  // { load(), draw(progress), poster(), busy(), prime() } — one object per segment,
  // built once at scene-build time from `rendererKind` ('video' default | 'imageseq').
  // Closes over the outer mountScrollWorld scope (reduce, isMobile, userReady, read,
  // primeVideo) exactly like the pre-refactor loadClip()/raf() did — no context object
  // threading, just the same functions moved onto a per-segment object.
  function createRenderer(kind, s) {
    return kind === 'imageseq' ? imageSequenceRenderer(s) : videoScrubRenderer(s);
  }

  // video-scrub-renderer — the original <video> blob-seek path, unchanged behavior,
  // extracted verbatim from the pre-refactor loadClip()/raf() body.
  function videoScrubRenderer(s) {
    return {
      load() {
        // Under prefers-reduced-motion we never load the clip at all — the still stays
        // up and simply cross-dissolves as you scroll. No scrubbed video motion, no
        // decode cost.
      ...
      draw(progress) {
        ...
        const t = clamp(progress, 0, 0.999) * dur;
        const eps = isMobile() ? 0.02 : 0.008;   // coarser seek step on phones = fewer decodes
        if (Math.abs(v.currentTime - t) > eps) { try { v.currentTime = t; } catch (e) {} }
      },
      poster() { return s.still || null; },
      // Never queue a seek while the decoder is still resolving the last one — on
      // phones a fast flick would otherwise pile up seeks and freeze the clip.
      busy() { return !!(s.video && s.video.seeking); },
      prime() { primeVideo(s.video); },
    };
  }

  // image-sequence-renderer — draws a preloaded WebP frame to a <canvas> per the
  // scrubbed progress. Own ~40-line drawImage pattern; no vendored scroller lib.
  function imageSequenceRenderer(s) {
    const N = s.frameCount || 0;
    const dir = s.framesDir || '';
    const frames = new Array(N);
    const requested = new Array(N).fill(false);
    // Progressive tier (opt-in via `framesLQ`): a tiny full-sequence loaded eagerly so the whole
    // scrub is instantly available at low-res, then each HQ frame upgrades it in place as it lands.
    const dirLQ = s.framesDirLQ || '';
    const framesLQ = new Array(N);
    const requestedLQ = new Array(N).fill(false);
    let canvas = null, ctx2d = null, firstPainted = false, lastIdx = 0;
    // Cache bound: eager frames (the initial batch loaded on load(), incl. frame 0
    // which doubles as the poster fallback) stay retained forever; everything else
    // outside the current scrub window gets released in releaseFar() below so a
    // long sequence doesn't fully decode into memory over a session.
    const EAGER = Math.min(10, N);
    const WINDOW = 5;
    // Vertical crop anchor for the manual cover-fit math below — matches the
    // <video>/<img> CSS `object-position: center 42%` so canvas frames crop to the
    // same focal point instead of dead-center (see drawFrame).
    const CROP_Y = 0.42;

    function frameUrl(i) { return dir + String(i + 1).padStart(4, '0') + '.webp'; }
    function frameUrlLQ(i) { return dirLQ + String(i + 1).padStart(4, '0') + '.webp'; }

    // LQ loader — tiny frames, kept resident (never released). Fired for the whole sequence on
    // load() so scrubbing works immediately before any HQ frame arrives.
    function requestFrameLQ(i) {
      if (!dirLQ || i < 0 || i >= N || requestedLQ[i]) return;
      requestedLQ[i] = true;
      const img = new Image(); img.decoding = 'async';
      img.onload = () => {
        framesLQ[i] = img;
        s.ready = true;  // any in-range LQ frame makes this beat drawable (draw loop gates on s.ready)
        if (canvas && !firstPainted && bestImg(lastIdx)) {
          drawFrame(bestImg(lastIdx)); firstPainted = true; s.el.classList.add('has-clip');
        }
      };
      img.onerror = () => { requestedLQ[i] = false; };
      img.src = frameUrlLQ(i);
    }

    function requestFrame(i) {
      if (i < 0 || i >= N || requested[i]) return;
      requested[i] = true;
      const img = new Image();
      img.decoding = 'async';
      img.onload = () => {
        frames[i] = img;
        s.ready = true;  // was `i === 0`; range-scoped beats never load frame 0, so key readiness off ANY loaded frame
        // Paint whichever index the shell is CURRENTLY targeting, not a hard frame 0
        // — if the scroll already moved past frame 0 before its (slower) network
        // load resolves, redrawing frame 0 here would stomp the frame the user is
        // actually looking at. If lastIdx's own frame isn't loaded yet either, this
        // is a no-op and the eventual onload for lastIdx paints it instead.
        if (canvas && !firstPainted && bestImg(lastIdx)) {
          drawFrame(bestImg(lastIdx)); firstPainted = true; s.el.classList.add('has-clip');
        } else if (canvas && frames[lastIdx] === img) {
          drawFrame(img);  // HQ upgrade for the frame currently on screen — sharpen in place
        }
      };
      // A failed load (network blip, 404) must not leave this index permanently
      // marked `requested` — that would leave a hole in the sequence forever, since
      // requestFrame() no-ops on an already-requested index. Clearing it lets the
      // next draw() pass (window still covers this index while nearby) retry.
      img.onerror = () => { requested[i] = false; };
      img.src = frameUrl(i);
    }

    // Release frames outside the eager set + the current ± WINDOW so a long scrub
    // session doesn't retain every visited frame forever (full-sequence memory
    // growth). Simple cap: keep it cheap, no LRU bookkeeping.
    ...
    // Manual object-fit:cover crop — <canvas> has no CSS object-fit, so we compute
    // the same "fill + crop" rectangle math the browser applies to <video>/<img>,
    // anchored at CROP_Y vertically (parity with their object-position: center 42%).
    // Best available image for an index: HQ if decoded, else the LQ stand-in.
    ...
    return {
      load() {
        if (s.loading || s.hasClip || !dir || !N) return;
        s.loading = true;
        ...
        resizeCanvas();
        s.canvas = canvas; s.hasClip = true;
        const lo = s.range ? s.range[0] : 0, hi = s.range ? s.range[1] : N - 1;
        // LQ: fetch the whole in-range span up front (tiny frames) so this beat is instantly
        // scrubbable at low-res; HQ then upgrades within the window as the user rests on a frame.
        if (dirLQ) for (let i = lo; i <= hi; i++) requestFrameLQ(i);
        for (let i = 0; i < EAGER; i++) requestFrame(lo + i);
      },
      draw(progress) {
        if (!canvas || !N) return;
        // `reverse` plays the frames backwards; `range:[lo,hi]` maps this section to a
        // sub-span of one shared frame dir (per-beat copy over a single continuous sequence).
        const p = s.reverse ? 1 - progress : progress;
        const lo = s.range ? s.range[0] : 0, hi = s.range ? s.range[1] : N - 1;
        const idx = clamp(Math.round(p * (hi - lo)) + lo, 0, N - 1);
        lastIdx = idx;
        for (let d = -5; d <= 5; d++) requestFrame(idx + d);
        releaseFar(idx);
        const paint = (img) => { drawFrame(img);
          if (!firstPainted) { firstPainted = true; s.el.classList.add('has-clip'); } };
        const bi = bestImg(idx);
        if (bi) { paint(bi); return; }
        // Fallback to the nearest available frame (HQ or LQ) so a fast flick never
        // flashes blank while the HQ window is still in flight.
        for (let d = 1; d <= 5; d++) {
          const a = bestImg(idx - d); if (a) { paint(a); return; }
          const b = bestImg(idx + d); if (b) { paint(b); return; }
        }
      },
      poster() { const lo = s.range ? s.range[0] : 0, hi = s.range ? s.range[1] : N - 1;
                 const i = s.reverse ? hi : lo;
                 return s.still || (N ? (dirLQ ? frameUrlLQ(i) : frameUrl(i)) : null); },
      busy() { return false; },        // drawImage is synchronous, never mid-seek
      prime() {},                      // no autoplay-policy concern for canvas draws
      resize() { resizeCanvas(); },
    };
  }
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      if (y > s.start - 1.6 * vh && y < s.end + 1.6 * vh) s.renderer.load();
  ...
  function raf() {
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      if (!s.hasClip || !s.ready || !s.renderer) continue;
      // Renderer-specific "can't accept an update right now" check (video: mid-seek;
      // imageseq: never). On phones a fast flick would otherwise pile up video seeks
      // and freeze the clip; cur keeps lerping, so we snap to the latest target the
      // moment it's free.
      if (s.renderer.busy()) continue;
      if (!s.visible && Math.abs(s.cur - s.target) < 0.002) continue;
      s.cur += (s.target - s.cur) * (reduce ? 1 : 0.18);
      // Full 0..1 range here — the video renderer clamps to 0.999 internally on its
      // own currentTime math (see videoScrubRenderer.draw); that clamp used to live
      // HERE instead, shared across every renderer, so the image-sequence renderer's
      // idx = round(progress*(N-1)) could never resolve to the true last frame
      // (N-1) for progress that would only ever cap at 0.999. Renderer-specific
      // ceilings belong on the renderer, not the shell.
      s.renderer.draw(clamp(s.cur, 0, 1));
    }
  ...
  function onFirstGesture() {
    if (userReady) return;
    userReady = true;
    SEGMENTS.forEach(s => { if (s.renderer && s.renderer.prime) s.renderer.prime(); });
  }
...
function injectCSS() {
  ...
  .sw-route{position:fixed;right:clamp(14px,2.4vw,30px);top:50%;z-index:40;transform:translateY(-50%);display:flex;flex-direction:column;align-items:center;gap:22px;padding:18px 10px;}
  ...
  .sw-route__dot{position:relative;border:0;background:transparent;cursor:pointer;width:14px;height:14px;display:flex;align-items:center;justify-content:center;}
  .sw-route__dot i{flex:0 0 9px;width:9px;height:9px;border-radius:50%;background:color-mix(in srgb,var(--sw-accent) 40%,transparent);transition:transform .3s,background .3s,box-shadow .3s;}
