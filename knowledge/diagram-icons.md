---
id: diagram-icons
description: "The monochrome 24x24 icon set for diagram and chart artifacts — 86 currentColor icons across 12 categories, with per-icon upstream attribution."
when: [diagram-icon-lookup, infrastructure-node-glyph, monochrome-svg-icon-set]
---

# Diagram Icons — the monochrome node set for diagram and chart artifacts

A monochrome 24×24 icon library for the infrastructure, platform, and data-stack nodes that
appear inside diagram and chart artifacts. 86 icons across 12 categories. Every icon draws with
`currentColor`, so it inherits ink from its parent group and adapts to the editorial skin or any
onboarded brand palette without being edited.

## Usage

Find the icon by its `### name` heading below and copy the fenced `<svg>` snippet into the
artifact. An icon pasted unmodified is already lint-clean — the work is positioning it, not
recolouring it.

### Colour comes from the parent, always

Icons inherit colour via `currentColor`. Set `color` on the parent group; never write a literal
`fill` or `stroke` onto the icon itself.

```svg
<g color="var(--color-ink-muted)" transform="translate(120,64)">
  <!-- paste the icon here; it inherits the group's colour -->
</g>
```

**This is a hard gate, not a preference.** `ui diagram lint` and `ui chart lint` reject literal
colours — hex, `rgb()`, `oklch()`, CSS keywords — in SVG presentation attributes through the
`hardcoded-svg-color` check. The permitted values are `currentColor`, `none`, and
`var(--color-…)`. Changing an icon's `stroke="currentColor"` to a literal is exactly what that
check exists to catch.

### Size and position

Default size is 24×24. Wrap in a transform to place and scale it:

```svg
<g transform="translate(x,y) scale(s)">…</g>
```

### Two styles — don't blend them casually

Generic icons are **stroked** (1.5px hairline, matching the rest of the diagram line work). Brand
silhouettes are **filled**. Mixing stroked and filled glyphs inside one diagram reads as an
inconsistency unless the split itself carries meaning (for example: generic roles stroked, named
vendor products filled).

### Repeated icons — paste once, then hoist

Paste inline for a one-off. When the same icon appears **more than twice** in one artifact, hoist
it into a symbol and reference it:

```svg
<defs>
  <symbol id="icon-server" viewBox="0 0 24 24">…</symbol>
</defs>
<use href="#icon-server" x="120" y="64" width="24" height="24" />
```

Same-document `#` references are permitted by the linters' `no-external-ref` check — that check
targets references that reach outside the artifact, not internal ones.

### Relationship to Lucide

`ui autofix` handles **Lucide** icons, which cover UI affordances in generated pages. This set
covers **infrastructure and platform nodes inside diagram and chart artifacts**. The precedence is
positional, not preferential:

| Where you are | Which set |
|---|---|
| Inside a diagram or chart artifact | This set |
| Inside a generated product UI | Lucide |

Never mix both inside one artifact. `ui autofix` never rewrites inline `<svg>`, so these icons pass
...
Every icon above carries its own `Source:` line naming the upstream project and licence. The set as
a whole draws from:

| Upstream | Licence | Covers |
|---|---|---|
| [Tabler Icons](https://github.com/tabler/tabler-icons) | MIT | The stroked generic icons — compute, people, network, data, Kubernetes, action, DevOps, file formats |
| [Simple Icons](https://github.com/simple-icons/simple-icons) | CC0 | The filled brand silhouettes |
| [log-z/logos](https://github.com/log-z/logos) | MIT | MySQL, Redis, StarRocks |
| [Devicon](https://github.com/devicons/devicon) | MIT | RStudio, SPSS |
| [Wikimedia Commons](https://commons.wikimedia.org/) | Public domain | SAS |

All of these permit redistribution, including inside this repository's MIT-licensed source. See
`THIRD_PARTY_LICENSES.md` at the repository root for the full detail.

### Deliberate exclusion — Stata

The **Stata** icon is **not** vendored here. Its upstream source (IcePanel / techicons.dev) carries
no verifiable licence grant, so redistributing it would be an unproven claim rather than a
documented one. It is omitted on purpose; do not re-add it from that source. If a Stata glyph is
ever needed, obtain it from a source with an explicit, checkable licence and add the attribution
alongside the icon.

A handful of remaining icons — `hop`, `pentaho`, `dagster` — still carry `Direct fetch — verify
license before use` markers inherited from the upstream capture. They are retained, but their
provenance has not been confirmed to the standard of the table above; verify before relying on them
in redistributed material.

### Trademarks

Brand logos remain the trademarks of their respective owners. Their inclusion in this icon set is
for documentation and illustrative use only. The presence of a brand mark in this repository does
