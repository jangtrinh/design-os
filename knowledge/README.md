...
| `scroll-cinema-direction.md` | **Directing a scroll-scrubbed camera flight** — the T6 gate (a scrubbed flight is authored assets, not a T3/T5 reveal), the architecture choice that must be made before generating anything (continuous forward take vs dive-plus-connector, and why B reverses direction at every seam), the two continuities a seam must hold (position = frame handoff, velocity = never reverse across a join), camera grammar under the motion-handoff contract, generator selection by frame-lock capability, the scrub-encode floor (seekability via blobs, not all-intra), the phone floors a ported engine must keep, and seam QA. Machine half: `video-scrub-no-reduced-motion` / `video-scrub-attrs` / `safe-area-viewport-fit` in `ui taste-lint`. |
