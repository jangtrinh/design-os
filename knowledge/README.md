...
| `verification-honesty.md` | **Why a false pass costs more than a crash** — the four rules that stop a check, a status flag, or a delegated agent from reporting success it did not earn: a checker MUST be able to **refuse** (not just pass/fail), totals MUST reconcile against what entered the pipeline, a status flag carries its SETTER's meaning rather than the reader's, and silence MUST be its own state instead of falling into the accepting branch. Read when designing any verdict another party will act on — a linter, a canvas check, a gate, an acceptance signal, or a delegated advisory call. |
...
| `diagram-craft.md` + `diagram-grammars/*.md` | **Native diagram craft** — decide whether a diagram beats prose, select exactly one of the nineteen supported grammars (using the collision-family precedence rules when two are close), inherit the project design system, author an accessible self-contained inline-SVG artifact, preserve `flow.json` as product-flow truth, account for every compression, then run the deterministic artifact floor and host taste critique. Read `diagram-craft.md` first, then exactly one grammar. |
| `chart-craft.md` + `chart-grammars/*.md` | **Native chart craft** — the sibling capability for standalone charts of quantities: the boundary against `generate`/`slides`/diagrams, routing across nine chart grammars, the honest-encoding floor (zero baselines, declared truncation, no dual axes, no fabricated data), token-bound series colour, and the accessible inline-SVG output contract. Read `chart-craft.md` first, then exactly one grammar. |
| `diagram-icons.md` | **The monochrome node icon set** — 86 24x24 `currentColor` icons across 12 categories for infrastructure and platform nodes inside diagram and chart artifacts, each with upstream attribution. Covers the lint rule (literal colours are rejected in presentation attributes), when to hoist a repeated icon into `<symbol>`/`<use>`, and the precedence split against Lucide (this set inside artifacts, Lucide in product UI). |
| `domain-packs/*.md` | **Optional domain vocabulary** — concrete component names and worked examples for a specific problem domain, kept out of the core grammars so those stay vendor-neutral. A grammar is fully usable with its pack absent; the pack only supplies naming. |
...
| `design-tells.md` | **The generated-UI tell family** — what a tell IS (an involuntary, machine-detectable sign a surface was made without design judgment; AI fingerprints are the salient subclass, not the definition), all 43 the `tell` family detects across surface, colour, type, motion and the rendered tier, and the routing rule that keeps the families apart: a broken rubric law goes to `taste`, a sign or fingerprint goes to `tell`. Most are advisory — a tell prints, it never fails a build. Read when adding a rule, or when deciding which family a new check belongs to. |
...
| `scroll-cinema-direction.md` | **Directing a scroll-scrubbed camera flight** — the T6 gate (a scrubbed flight is authored assets, not a T3/T5 reveal), the architecture choice that must be made before generating anything (continuous forward take vs dive-plus-connector, and why B reverses direction at every seam), the two continuities a seam must hold (position = frame handoff, velocity = never reverse across a join), camera grammar under the motion-handoff contract, generator selection by frame-lock capability, the scrub-encode floor (seekability via blobs, not all-intra), the phone floors a ported engine must keep, and seam QA. Machine half: `video-scrub-no-reduced-motion` / `video-scrub-attrs` / `safe-area-viewport-fit` / `video-poster-missing` in `ui taste-lint`, plus `ui scrub-lint <file.mp4>` for the encode floor (ISO-BMFF box walk — faststart, audio, GOP length). |
| `gflow-hand.md` | **Driving Google Flow (Veo/Imagen) through the gflow hand** — the golden path is `ui gflow i2v` (verify-then-spend: it confirms a new MP4 landed, then extracts that video's real last frame as the next leg's seed), why `gflow video chain` and `--end-frame` are excluded, the interactive auth contract, model choice reasoning (the fast model strands less often than the sharp one), and how to recover a stranded render by media id instead of paying for it twice. Read before generating authored video legs; `scroll-cinema-direction.md` owns what makes the flight good. |
...
1. `need-routing.md` + `ui knowledge activate` — classify the requested artifact with evidence; use HTML generation only when the receipt authorizes qualified `web-marketing → generate → html`
...
**Generate or decompose original raster assets for a website** —
`asset-production-orchestration.md`: compile the asset request, lock the reference, route hard and
soft transparency correctly, preserve raw/processed/curated custody, then recompose and publish
