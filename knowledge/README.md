...
| `diagram-craft.md` + `diagram-grammars/{product-flow,architecture,sequence}.md` | **Native diagram craft** — decide whether a diagram beats prose, select exactly one supported grammar, inherit the project design system, author an accessible self-contained inline-SVG artifact, preserve `flow.json` as product-flow truth, account for every compression, then run the deterministic artifact floor and host taste critique. Read `diagram-craft.md` first, then exactly one grammar. |
